...
[![mAP](https://img.shields.io/badge/mAP@50-82%25_val-1d9e75?style=flat-square)]()

**82%+ mAP@50 · 88.4% Precision · 94% Construction Waste AP · 22,031 images**
...
The project ran **12+ training iterations** starting from 63.6% mAP and reached **82%+ mAP@50** through a strategy of systematic data collection and quality improvement — not architecture changes. Two parallel model tracks were developed: **YOLO26 Fast** (via Roboflow managed training) and **YOLOv8m** (via Kaggle P100 GPU), both converging at 82%+.
...
| | |
|---|---|
| **82%+ mAP@50 (val)** | Maintained across two consecutive model versions (Neat Now 3 & 4) and two architectures |
...
| **88.4% Precision** | Best precision achieved — Neat Now 4 |
...
Two models are maintained in parallel:

| Parameter | YOLO26 (Roboflow) | YOLOv8m (Kaggle) |
|-----------|-------------------|------------------|
| Model | YOLO26 Object Detection (Fast) | YOLOv8m |
| Input resolution | 640 × 640 px | 640 × 640 px |
| Training epochs | 150–155 (full convergence) | 90 (early stop) |
| Early-stop patience | 30 | 30 |
| Batch size | Managed | 32 |
| AMP | Yes | Yes (FP16) |
| Cache | — | RAM |
| Platform | Roboflow Managed | Kaggle P100 GPU |
| Base weights | COCO pretrained | COCO pretrained |
| Best val mAP | 81.7% | 82.5% |
...
| **Neat Now 4** | **Apr 5** | **~150** | **81.7%** | — | **+2.2pp val; precision 88.4%; recall 72.2%** |
| **YOLOv8m (Kaggle)** | **Apr 5** | **90** | **82.5%** | — | **batch=32+cache=ram; 1 session; mAP50-95=65.3%** |
...
### Key lessons from 12+ iterations
...
|---|---|---|
| `waste` class (57–58%) | Semantically ambiguous catch-all — stuck across 6 versions and 3 architectures; 47% of objects missed as background | Re-label ambiguous images to specific classes |
...
| Center-bias in dataset | Majority of annotations are centre-framed; real-world objects appear anywhere | Collect edge-framed images |
...
- [x] YOLOv8m Kaggle experiment — 82.5% val, 65.3% mAP50-95, 1 session
- [x] ONNX export — 98.8 MB, smoke test passed
...
| Current best model | Neat Now 4 — YOLO26 Fast (81.7% val mAP) |
...
```bibtex
@misc{neat-now-2026,
  title   = {Neat Now: 9-Class Urban Waste Detection using YOLO26 and YOLOv8m},
  author  = {Muhammad Qasim Javed},
  year    = {2026},
  url     = {https://github.com/Al-Qasim/neat-now},
  note    = {YOLO26 Fast + YOLOv8m · 82\%+ mAP@50 · 9 classes · 22,031 images}
}
```
...
<sub>Last updated: April 5, 2026 · Current best: Neat Now 4 (81.7% val) + YOLOv8m Kaggle (82.5% val) · Dataset: Neat Now 3 (22,031 images)</sub>
</div>
