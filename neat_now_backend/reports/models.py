...
class Report(models.Model):
    ...
    STATUS_CHOICES = [
        ('Pending', 'Pending'),
        ('Assigned', 'Assigned'),  # Worker accepted but not started (shows as "Pending" in frontend)
        ('In Progress', 'In Progress'),  # Worker started the task
        ('Resolved', 'Resolved'),
        ('Rejected', 'Rejected'),
    ]
    ...
    status = models.CharField(
        max_length=15,  # Increased to accommodate "In Progress" (11 chars)
    ...
    accepted_at = models.DateTimeField(null=True, blank=True, db_index=True, help_text='When worker accepted the report (for consistent task numbering)')
